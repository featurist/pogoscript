...
module.exports = function (listOfTerminals) {
  ...
  return terms.oldTerm(function () {
    ...
    this.expression = function () {
      ...
      if (head.hasName()) {
        if (this.hasArguments()) {
          return this.wrap(terms.functionCall(terms.variable(head.name(), {couldBeMacro: false, location: this.location()}), this.arguments(), {optionalArguments: this.optionalArguments(), async: this.isAsyncCall(), future: this.isFutureCall()}));
        } else {
        ...
        if (!this.hasTail() && this.arguments().length === 1 && !this.isCall() && !this.head().isCall()) {
          ...
          return this.wrap(terms.functionCall(this.arguments()[0], this.arguments().slice(1), {async: this.isAsyncCall(), future: this.isFutureCall()}));
        }
      }
    };
    ...
    this.objectOperationExpression = function (object) {
      ...
      if (head.hasName()) {
        if (this.hasArguments()) {
          return this.wrap(terms.methodCall(object, head.name(), this.arguments(), {optionalArguments: this.optionalArguments(), async: this.isAsyncCall(), future: this.isFutureCall()}));
        } else {
        ...
        if (!this.hasTail() && !head.isCall() && !this.isAsyncCall()) {
          ...
          return this.wrap(terms.functionCall(terms.indexer(object, this.arguments()[0]), this.arguments().slice(1), {async: this.isAsyncCall(), future: this.isFutureCall()}));
        }
      }
    };

    this.wrap = function (term) {
      if (this.isCallbackCall()) {
        var promiseFunction = terms.moduleConstants.defineAs(
          ['promise'],
