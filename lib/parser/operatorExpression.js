(function() {
    ...
    module.exports = function(terms) {
        var self = this;
        var operatorStack, operatorsInDecreasingPrecedenceOrder, operatorTable, normaliseOperatorName, createOperatorCall;
        ...
        normaliseOperatorName = function(name) {
            var match;
            ...
            expression: function() {
                ...
                if (self.arguments.length > 1) {
                    ...
                    for (n = 0; n < self.name.length; n = n + 1) {
                        op = operatorTable.findOp(self.name[n]);
                        poppedOps = operators.push(op);
                        applyOperators(poppedOps);
                        operands.unshift(self.arguments[n + 1].expression());
                    }
                    poppedOps = operators.pop();
                    applyOperators(poppedOps);
                    return operands[0];
                } else {
