((function() {
    ...
    module.exports = function(terms) {
        ...
        return ifExpression = function(cases, elseBody) {
            var anyAsyncCases, asyncClosureFor, caseForConditionAndBody, casesList, asyncIfElseIfElseFunction, asyncIfElseFunction, asyncIfFunction;
            anyAsyncCases = _.any(cases, function(_case) {
                return _case.body.isAsync;
            });
            if (anyAsyncCases || elseBody && elseBody.isAsync) {
                asyncClosureFor = function(body) {
                    var closure;
                ...
                if (cases.length > 1) {
                    caseForConditionAndBody = function(condition, body) {
                        return terms.hash([ terms.hashEntry([ "condition" ], condition), terms.hashEntry([ "body" ], asyncClosureFor(body)) ]);
                    };
                    ...
                    return terms.functionCall(asyncIfElseFunction, [ cases[0].condition, asyncClosureFor(cases[0].body), asyncClosureFor(elseBody) ], {
                        async: true
                    });
                } else {
                    asyncIfFunction = terms.moduleConstants.defineAs([ "async", "if" ], terms.javascript(asyncControl.if.toString()));
                    return terms.functionCall(asyncIfFunction, [ cases[0].condition, asyncClosureFor(cases[0].body) ], {
                        async: true
                    });
                }
            } else {
