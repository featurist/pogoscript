    ...
    module.exports = function(terms) {
        ...
        return terms.term({
            constructor: function(parameters, body, gen1_options) {
                var returnLastStatement, redefinesSelf, async, self;
                ...
                return self.rewriteResultTermToReturn(returnLastStatement);
            },
            rewriteResultTermToReturn: function(returnLastStatement) {
                var self;
                self = this;
                if (returnLastStatement && !self.body.isAsync) {
                    return self.body.rewriteLastStatementToReturn({
                        async: self.isAsync
                    });
                }
            },
            blockify: function(parameters, optionalParameters, gen2_options) {
                var async, self;
            ...
            rewriteStatements: function(clone) {
                var self, c, callbackFunction, asyncRewrite;
                self = this;
                c = clone();
                callbackFunction = terms.callbackFunction;
                asyncRewrite = c.body.rewriteAsyncCallbacks({
                    returnLastStatement: self.returnLastStatement,
                    forceAsync: self.isAsync,
                    callbackFunction: callbackFunction
                });
                c.body = asyncRewrite.statements;
                if (asyncRewrite.isAsync) {
                    c.isAsync = true;
                    c.asyncCallbackFunction = callbackFunction;
                }
                return c;
            }
        });
