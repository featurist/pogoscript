...
module.exports (cg) =
    if expression term = class extending (cg.term class) {
        ...
        generate java script statement (buffer, scope, generateReturnStatements) =
    ...
    if expression (args, ...) = new (if expression term (args, ...))
