...
exports.generate = function(items, block, continuation) {
  ...
  if (items.length > 0) {
    for (var n = 0; n < items.length; n++) {
      block(n, items[n], function (error, result) {
        if (error) {
          wasError = true;
          continuation(error);
        }
...
exports.sortResults = function(block, continuation) {
  var indexes = [];
  var items = {};

  block(function (item, index) {
    indexes.push(index);
    items[index] = item;
  }, function (error) {
    if (error) {
...
exports.sortResultRanges = function(block, continuation) {
  var indexes = [];
  var ranges = {};

  block(function (range, index) {
    indexes.push(index);
    ranges[index] = range;
  }, function (error) {
    if (error) {
      ...
      for (n = 0; n < indexes.length; n++) {
        results.push.apply(results, ranges[indexes[n]]);
      }
