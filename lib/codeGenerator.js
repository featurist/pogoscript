...
exports.optional = function (options, name, defaultValue) {
  return this.oldTerm(function () {
    ...
    this.generateJavaScript = function (buffer, scope) {
      ...
      buffer.write('&&');
      this.options.generateJavaScript(buffer, scope);
      buffer.write(".hasOwnProperty('" + codegenUtils.concatName(this.name) + "')&&");
      this.options.generateJavaScript(buffer, scope);
      buffer.write("." + codegenUtils.concatName(this.name) + "!==void 0)?");
      this.options.generateJavaScript(buffer, scope);
      buffer.write('.' + codegenUtils.concatName(this.name) + ':');
...
exports.methodCall = function (object, name, args, optionalArgs) {
  var splattedArgs = codegenUtils.splattedArguments(this, args, optionalArgs);
  ...
  if (splattedArgs) {
    ...
    return this.oldTerm(function () {
      ...
      this.generateJavaScript = function (buffer, scope) {
        ...
        buffer.write(codegenUtils.concatName(this.name));
        buffer.write('(');
        codegenUtils.writeToBufferWithDelimiter(codegenUtils.argsAndOptionalArgs(this.cg, this.methodArguments, this.optionalArguments), ',', buffer, scope);
        buffer.write(')');
      };
    });
  }
