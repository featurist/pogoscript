...
var newTerm = require('./terms').term;
...
var ExpressionPrototype = new newTerm();
...
exports.term = function (members) {
  if (!this.thisiscodegen) {
    throw new Error('no cg!');
  }
  
  if (arguments.length > 1) {
    throw new Error('too many args to term!');
  }
  ...
  var constructor = function () {
    this.cg = cg;
    //Object.defineProperty(this, 'cg', {value: cg});
    members.call(this);
  };
...
exports.ifCases = function (cases, _else) {
  return this.term(function () {
    ...
    this.generateJavaScript = function (buffer, scope) {
      this.cg.functionCall(this.cg.subExpression(this.cg.block([], this.cg.statements([this]))), []).generateJavaScript(buffer, scope);
    };
...
exports.forStatement = function(init, test, incr, stmts) {
  return this.term(function () {
    ...
    this.scopedBody = function () {
      ...
      var cg = this.cg;
      var statements = this.statements.clone(function (term) {
        if (term.isReturn) {
      ...
      loopStatements.push(this.cg.ifCases([[this.cg.subExpression(this.cg.functionCall(this.cg.block([this.indexVariable], statements, {returnLastStatement: false}), [this.indexVariable])), this.cg.statements([this.cg.returnStatement(forResultVariable)])]]));
