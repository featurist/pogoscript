(function() {
    ...
    module.exports = function(terms) {
        ...
        return module = function(statements, gen2_options) {
            ...
            var scope, args, methodCall, call, errorVariable;
            ...
            if (inScope) {
                ...
                call = function() {
                    if (statements.isAsync) {
                        ...
                        terms.functionCall(terms.variable([ "set", "timeout" ]), [ terms.closure([], terms.statements([ terms.throwStatement(errorVariable) ])), terms.integer(0) ]);
                        return terms.methodCall(methodCall, [ "then" ], [ terms.nil(), terms.closure([ errorVariable ], terms.statements([ terms.functionCall(terms.variable([ "set", "timeout" ]), [ terms.closure([], terms.statements([ terms.throwStatement(errorVariable) ])), terms.integer(0) ]) ])) ]);
                    } else {
                        return methodCall;
                    }
                }();
