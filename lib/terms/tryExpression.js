            ...
            if (body.returnsPromise || catchBody && catchBody.returnsPromise || finallyBody && finallyBody.returnsPromise) {
                if (catchBody) {
                    if (finallyBody) {
                        ...
                        return terms.resolve(terms.methodCall(body.promisify(), [ "then" ], [ terms.nil(), terms.closure([ catchParameter ], catchBody) ]), {
                            alreadyPromise: true
                        });
                    }
                } else if (finallyBody) {
                    result = terms.generatedVariable([ "result" ]);
                    return terms.resolve(terms.methodCall(body.promisify(), [ "then" ], [ terms.closure([ result ], terms.statements([ terms.methodCall(finallyBody.promisify(), [ "then" ], [ terms.closure([], terms.statements([ result ])) ]) ])) ]), {
                        alreadyPromise: true
                    });
                } else {
                    return terms.resolve(body);
