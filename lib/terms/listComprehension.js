        ...
        generator = function(expression) {
            return {
                ...
                generate: function(isAsync, result, index) {
                    var self = this;
                    var sortResults, generate, innerResult, innerIndex, asyncStatements, generateCall, sortCall;
                    if (isAsync) {
                        sortResults = function() {
                            if (self.next.hasGenerator()) {
                                return terms.moduleConstants.defineAs([ "sort", "result", "ranges" ], terms.javascript(asyncControl.sortResultRanges.toString()));
                            } else {
                                return terms.moduleConstants.defineAs([ "sort", "results" ], terms.javascript(asyncControl.sortResults.toString()));
                            }
                        }();
                        ...
                        generateCall = terms.functionCall(generate, [ self.collection, terms.closure([ innerIndex, self.iterator ], asyncStatements) ], {
                            async: true
                        });
                        sortCall = terms.functionCall(sortResults, [ terms.closure([ innerResult ], terms.asyncStatements([ generateCall ])) ], {
                            async: true
                        });
                        if (result) {
                            return [ terms.functionCall(result, [ sortCall, index ]) ];
                        } else {
                            return [ sortCall ];
                        }
                    } else {
