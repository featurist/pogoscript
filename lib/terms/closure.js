(function() {
    ...
    module.exports = function(terms) {
        var self = this;
        var optionalParameters, optional, asyncParameters, containsSplatParameter, createSplatParameterStrategyFor, createOptionalParameterStrategyFor, promisifyBody;
        ...
        asyncParameters = function(closure, next) {
            return {
                parameters: function() {
                    var self = this;
                    if (closure.isAsync) {
                        return next.parameters().concat([ terms.continuationFunction ]);
                    } else {
        ...
        promisifyBody = function(body) {
            var promiseFunction;
            promiseFunction = terms.moduleConstants.defineAs([ "promise" ], terms.javascript(asyncControl.promise.toString()));
            terms.moduleConstants.defineAs([ "Promise" ], terms.javascript('require("es6-promise").Promise'), {
                generated: false
            });
            return terms.statements([ terms.returnStatement(terms.functionCall(promiseFunction, [ terms.closure([ terms.resolveFunction ], body, {
                inPromise: true
            }) ])) ]);
        };
        return terms.term({
            constructor: function(parameters, body, gen1_options) {
                var self = this;
                var optionalParameters, returnLastStatement, redefinesSelf, async, definesModuleConstants, inPromise;
                ...
                inPromise = gen1_options !== void 0 && Object.prototype.hasOwnProperty.call(gen1_options, "inPromise") && gen1_options.inPromise !== void 0 ? gen1_options.inPromise : false;
                self.isBlock = true;
                self.isClosure = true;
                self.parameters = parameters;
                self.body = function() {
                    if (body.isAsync && !inPromise) {
                        return promisifyBody(body);
                    } else {
                        return body;
                    }
                }();
