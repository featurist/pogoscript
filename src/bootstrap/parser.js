(function() {
    ...
    grammar = {
        lex: {
            ...
            rules: [ [ "\\s+", "/* ignore whitespace */" ], [ "[0-9]+\\.[0-9]+", "return 'float';" ], [ "[0-9]+", "return 'integer';" ], [ "@" + identifierPattern, "return 'argument';" ], [ "@:" + identifierPattern, "return 'self_argument';" ], [ "\\?" + identifierPattern, "return 'parameter';" ], [ identifierPattern, "return 'identifier';" ], [ "\\??\\(", "if (yy.interpolation.interpolating()) {yy.interpolation.openBracket()} return yytext;" ], [ "\\)", "if (yy.interpolation.interpolating()) {yy.interpolation.closeBracket(); if (yy.interpolation.finishedInterpolation()) {this.popState(); this.popState(); yy.interpolation.stopInterpolation()}} return ')';" ], [ "{", "return '{';" ], [ "}", "return '}';" ], [ "\\[", "return '[';" ], [ "\\]", "return ']';" ], [ "\\\\[{}.]", "return yytext.substring(1);" ], [ "[:=,?!.@`~#$%^&*+<>/?\\\\|-]+", "return yy.lexOperator(yytext);" ], [ "$", "return 'eof';" ], [ "'([^']*'')*[^']*'", "return 'string';" ], [ '"', "this.begin('interpolated_string'); return 'start_interpolated_string';" ], [ [ "interpolated_string" ], "@", "this.begin('interpolated_string_terminal'); return 'interpolated_string_terminal_start';" ], [ [ "interpolated_string_terminal" ], identifierPattern, "this.popState(); return 'identifier';" ], [ [ "interpolated_string_terminal" ], "\\(", "yy.interpolation.startInterpolation(); this.begin('INITIAL'); return '(';" ], [ [ "interpolated_string" ], '"', "this.popState(); return 'end_interpolated_string';" ], [ [ "interpolated_string" ], "\\\\[{}.]", "/* ignore */" ], [ [ "interpolated_string" ], "\\\\\\\\\\\\\\\\", "return 'escaped_escape_interpolated_string_body';" ], [ [ "interpolated_string" ], "\\\\\\\\.", "return 'escaped_interpolated_string_body';" ], [ [ "interpolated_string" ], '[^"@\\\\]*', "return 'interpolated_string_body';" ], [ ".", "return 'non_token';" ] ]
        },
        ...
        bnf: {
            ...
            terminal: [ [ "( statement )", "$$ = $2;" ], [ "?( statement )", "$$ = yy.parameter($2);" ], [ "{ statements }", "$$ = yy.loc(yy.block([], $2), @$);" ], [ "=> { statements }", "$$ = yy.loc(yy.block([], $3, {redefinesSelf: true}), @$);" ], [ "[ statements_list ]", "$$ = yy.loc(yy.list($2), @$);" ], [ "# { hash_entries }", "$$ = yy.loc(yy.hash($3), @$);" ], [ "float", "$$ = yy.loc(yy.float(parseFloat(yytext)), @$);" ], [ "integer", "$$ = yy.loc(yy.integer(parseInt(yytext)), @$);" ], [ "identifier", "$$ = yy.loc(yy.identifier(yytext), @$);" ], [ "argument", "$$ = yy.loc(yy.variable([yytext.substring(1)]), @$);" ], [ "self_argument", "$$ = yy.loc(yy.fieldReference(yy.variable(['self']), [yytext.substring(2)]), @$);" ], [ "parameter", "$$ = yy.loc(yy.parameter(yy.variable([yytext.substring(1)])), @$);" ], [ "string", "$$ = yy.loc(yy.string(yy.normaliseString(yytext)), @$);" ], [ "interpolated_string", "$$ = yy.loc($1, @$);" ] ],
            ...
            interpolated_string_component: [ [ "interpolated_string_terminal_start interpolated_terminal", "$$ = $2;" ], [ "interpolated_string_body", "$$ = yy.string($1);" ], [ "escaped_interpolated_string_body", "$$ = yy.string($1.substring(1));" ], [ "escaped_escape_interpolated_string_body", "$$ = yy.string($1.substring(2));" ] ]
        }
    };
