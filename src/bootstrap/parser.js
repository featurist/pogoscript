(function() {
    ...
    grammar = {
        ...
        bnf: {
            ...
            terminal: [ [ "( statement )", "$$ = $2;" ], [ "{ statements }", "$$ = yy.loc(yy.block([], $2), @$);" ], [ "=> { statements }", "$$ = yy.loc(yy.block([], $3, {redefinesSelf: true}), @$);" ], [ "[ statements_list ]", "$$ = yy.loc(yy.list($2), @$);" ], [ "# { hash_entries }", "$$ = yy.loc(yy.hash($3), @$);" ], [ "float", "$$ = yy.loc(yy.float(parseFloat(yytext)), @$);" ], [ "integer", "$$ = yy.loc(yy.integer(parseInt(yytext)), @$);" ], [ "identifier", "$$ = yy.loc(yy.identifier(yytext), @$);" ], [ "argument", "$$ = yy.loc(yy.variable([yytext.substring(1)]), @$);" ], [ "self_argument", "$$ = yy.loc(yy.fieldReference(yy.variable(['self']), [yytext.substring(2)]), @$);" ], [ "parameter", "$$ = yy.loc(yy.parameter([yytext.substring(1)]), @$);" ], [ "string", "$$ = yy.loc(yy.string(yy.normaliseString(yytext)), @$);" ], [ "interpolated_string", "$$ = yy.loc($1, @$);" ], [ "no_arg", "$$ = yy.loc(yy.noArgSuffix(), @$);" ] ],
