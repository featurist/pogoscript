(function() {
    ...
    grammar = {
        lex: {
            ...
            rules: [ [ "\\s+", "/* ignore whitespace */" ], [ "[0-9]+\\.[0-9]+", "return 'float';" ], [ "[0-9]+", "return 'integer';" ], [ "@" + identifierPattern, "return 'argument';" ], [ "@:" + identifierPattern, "return 'self_argument';" ], [ "\\?" + identifierPattern, "return 'parameter';" ], [ identifierPattern, "return 'identifier';" ], [ "\\??\\(", "if (yy.interpolation.interpolating()) {yy.interpolation.openBracket()} return yytext;" ], [ "\\)", "if (yy.interpolation.interpolating()) {yy.interpolation.closeBracket(); if (yy.interpolation.finishedInterpolation()) {this.popState(); this.popState(); yy.interpolation.stopInterpolation()}} return ')';" ], [ "{", "return '{';" ], [ "}", "return '}';" ], [ "\\[", "return '[';" ], [ "\\]", "return ']';" ], [ "\\\\[.]", "return yytext.substring(1);" ], [ "\\\\[{]", "if (!yy.stringBrackets) {return yytext.substring(1);} else {++yy.stringBrackets;}" ], [ "\\\\[}]", "if (!yy.stringBrackets) {return yytext.substring(1);} else {--yy.stringBrackets;}" ], [ "([:=,?!.@`~#$%^&*+<>/?|-]|\\\\)+", "return yy.lexOperator(yytext);" ], [ "$", "return 'eof';" ], [ "'([^']*'')*[^']*'", "return 'string';" ], [ '"', "this.begin('interpolated_string'); return 'start_interpolated_string';" ], [ [ "interpolated_string" ], "@", "this.begin('interpolated_string_terminal'); return 'interpolated_string_terminal_start';" ], [ [ "interpolated_string_terminal" ], identifierPattern, "this.popState(); return 'identifier';" ], [ [ "interpolated_string_terminal" ], "\\(", "yy.interpolation.startInterpolation(); this.begin('INITIAL'); return '(';" ], [ [ "interpolated_string" ], '"', "this.popState(); return 'end_interpolated_string';" ], [ [ "interpolated_string" ], "\\\\[.]", "/* ignore */" ], [ [ "interpolated_string" ], "\\\\[{]", "yy.stringBrackets++" ], [ [ "interpolated_string" ], "\\\\[}]", "yy.stringBrackets--;" ], [ [ "interpolated_string" ], "\\\\\\\\\\\\\\\\", "return 'escaped_escape_interpolated_string_body';" ], [ [ "interpolated_string" ], "\\\\\\\\.", "return 'escaped_interpolated_string_body';" ], [ [ "interpolated_string" ], '[^"@\\\\]*', "return 'interpolated_string_body';" ], [ ".", "return 'non_token';" ] ]
        },
