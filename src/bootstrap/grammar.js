((function() {
    ...
    exports.grammar = {
        ...
        bnf: {
            ...
            basic_expression_list: [ [ "basic_expression_list ; terminal_list", "$1.push($3); $$ = $1;" ], [ "terminal_list", "$$ = [$1];" ] ],
            ...
            async_operator: [ [ "!", "$$ = yy.terms.loc(yy.terms.asyncArgument(), @$);" ] ],
            terminal: [ [ "( arguments_list )", "$$ = yy.terms.loc(yy.terms.argumentList($arguments_list), @$);" ], [ "@ ( parameter_list )", "$$ = yy.terms.loc(yy.terms.parameters($3), @$);" ], [ "block_start statements }", "$$ = yy.terms.loc(yy.terms.block([], $2), @$);" ], [ "=> block_start statements }", "$$ = yy.terms.loc(yy.terms.block([], $3, {redefinesSelf: true}), @$);" ], [ "[ statements_list ]", "$$ = yy.terms.loc(yy.terms.list($2), @$);" ], [ "{ hash_entries }", "$$ = yy.terms.loc(yy.terms.hash($2), @$);" ], [ "float", "$$ = yy.terms.loc(yy.terms.float(parseFloat(yytext)), @$);" ], [ "integer", "$$ = yy.terms.loc(yy.terms.integer(parseInt(yytext)), @$);" ], [ "identifier", "$$ = yy.terms.loc(yy.terms.identifier(yytext), @$);" ], [ "string", "$$ = yy.terms.loc(yy.terms.string(yy.terms.unindent(@$.first_column + 1, yy.terms.normaliseString(yytext))), @$);" ], [ "reg_exp", "$$ = yy.terms.loc(yy.terms.regExp(yy.terms.parseRegExp(yy.terms.unindent(@$.first_column + 1, yytext))), @$);" ], [ "interpolated_string", "$$ = yy.terms.loc($1, @$);" ], [ "...", "$$ = yy.terms.loc(yy.terms.splat(), @$);" ] ],
            block_start: [ [ "@ {", "$$ = '@{'" ], [ "@{", "$$ = '@{'" ] ],
